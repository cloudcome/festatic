 ...
 * 再次修正了AJAX格式的问题
 * 2014年5月1日17:27:15
 * 去除了默认的上传脚本，修改为调用 jquery-upload
 * 2014年5月15日00:10:20
 * 再次规范了JSON格式
 * 2014年5月21日01:30:54
 * 增加pre功能
...
(function($) {
    ...
    $.Redactor.opts = {
        // 插入代码，pre 标签的 className
        preClass: "prettyprint linenums",

        // 回调
        insertPreBeforeCallback: false,
        insertPreAfterCallback: false,
        syncBeforeCallback: false,
        syncAfterCallback: false,
        ...
        autosaveErrorCallback: false,
        dropdownShowCallback: false,
        dropdownShownCallback: false,
        dropdownHideCallback: false,
        execCommandCallback: false,
        pasteBeforeCallback: false,
        pasteAfterCallback: false,
        imageUploadErrorCallback: false,
        imageGetErrorCallback: false,
        imageDeleteCallback: false,
        imageUploadCallback: false,
        modalOpenedCallback: false,
        modalClosedCallback: false,
        ...
        tabFocus: true,
        air: false,
        airButtons: ["formatting", "bold", "italic", "underline", "deleted", "pre", "code", "unorderedlist", "orderedlist"],
        toolbar: true,
        toolbarFixed: false,
        ...
        buttonSource: true,
        buttons: ["formatting", "bold", "italic", "underline", "deleted", "pre", "code", "unorderedlist", "orderedlist", "image", "video", "file", "table", "link", "alignment", "|", "horizontalrule", "html"],
        buttonsHideOnMobile: [],
        ...
        formattingTags: ["p", "blockquote", "code", "h1", "h2", "h3", "h4", "h5", "h6"],
        linebreaks: false,
        paragraphy: true,
        convertDivs: true,
        convertLinks: true,
        ...
        lang: {
            ...
            choose: "选择图片",
            or_choose: "或选择",
            drop_file_here: "将文件拖拽至此区域或单击选择文件",
            image_get_net_error: "获取图片列表出现网络错误",
            image_upload_net_error: "上传图片出现网络错误",
            autosave_net_error: "自动保存出现网络错误",
            align_left: "左对齐",
            align_center: "居中",
            align_right: "右对齐",
            align_justify: "两端对齐",
    ...
    Redactor.fn = $.Redactor.prototype = {
        ...
        init: function(el, options) {
            ...
            this.opts.curLang = this.opts.lang;
        ...
        syncClean: function(html) {
            ...
            html = this.syncGalleryLazyLoad(html);
            html = html.replace(/&amp;/gi, "&");
        ...
        syncGalleryLazyLoad: function(html, img) {
            if (!this.opts.galleryLazyLoad) {
                return html;
            }
            var matches = [];
            matches.push(html.match(/<div class="fotorama">([\w\W]*?)<\/div>/gi));
            matches.push(html.match(/<div class="royalSlider rsDefault">([\w\W]*?)<\/div>/gi));
            for (var i in matches) {
                html = this.convertGalleryLazyLoad(html, matches[i], img);
            };
            return html;
        },
        reSyncGalleryLazyLoad: function(html) {
            return this.syncGalleryLazyLoad(html, true);
        },
        convertGalleryLazyLoad: function(html, matches, img) {
            if (matches) {
                $.each(matches, function(i, s) {
                    var str = "";
                    if (img === true) {
                        str = s.replace(/<a(.*?)href="(.*?)"(.*?)><\/a>/gi, "<img$1src=\"$2\"$3>");
                    } else {
                        str = s.replace(/<img(.*?)src="(.*?)"(.*?)>/gi, "<a href=\"$2\"$1$3></a>");
                    };
        ...
        buildBindKeyboard: function() {
            this.dblEnter = 0;
            if (this.opts.dragUpload && this.opts.imageUploadUrl !== false) {
        ...
        buildEventKeydown: function(e) {
            ...
            if (key === this.keyCode.BACKSPACE) {
                this.buildEventKeydownBackspace(e, current);
            }
        },
        ...
        toggleVisual: function() {
            ...
            if (this.modified) {
                html = this.reSyncGalleryLazyLoad(html);
            ...
            this.autosaveInterval = setInterval($.proxy(function() {
                ...
                if (savedHtml !== html) {
                    ...
                    $.ajax({
                        ...
                        type: "post",
                        data: $.extend({}, this.opts.autosaveData, data),
                        ...
                        error: $.proxy(function() {
                            this.callback("autosaveError", false, {
                                code: 0,
                                msg: this.opts.curLang.image_get_net_error
                            });
                        }, this)
                    });
                };
        ...
        buttonBuild: function(btnName, btnObject, buttonImage) {
            var $button = $("<a href=\"javascript:;\" title=\"" + btnObject.title + "\" tabindex=\"-1\" class=\"re re-" + btnName + "\"></a>");
        ...
        formatInline: function(tag) {
            this.inlineFormat(tag);
        },
        ...
        tableInsert: function() {
            ...
            var rows = $("#redactor_table_rows").val(),
                ...
                $table = $("<table id='table" + tableId + "' class='table table-hover table-striped table-bordered'><tbody></tbody></table>"),
                i, $row, z, $column;
            for (i = 0; i < rows; i++) {
                $row = $("<tr></tr>");
                for (z = 0; z < columns; z++) {
        ...
        preShow: function() {
            this.selectionSave();
            this.modalInit(this.opts.curLang.pre, this.opts.modal_pre, 600, $.proxy(function() {
                $("#redactor_insert_pre_btn").click($.proxy(this.preInsert, this));
                setTimeout(function() {
                    $("#redactor_insert_pre_area").focus();
                }, 200);
            }, this));
        },
        preInsert: function() {
            ...
            var p = document.createElement('p'),
                data = '<pre class="' + this.opts.preClass + '">' + $(p).text($("#redactor_insert_pre_area").val()).html() + '</pre>',
                current = this.getBlock() || this.getCurrent();

            data = this.callback("insertPreBefore", false, data);
            ...
            this.callback("insertPreAfter", false, data);
            p = null;
        },
        videoShow: function() {
            this.selectionSave();
        ...
        imageShow: function() {
            ...
            var callback = $.proxy(function() {
                var $imageBox = $("#redactor_image_box");
                if (!$imageBox.data('click')) {
                    $imageBox.data('click', 1).on('click', '.redactor_image_box li', function() {
                        _this.imageClick.call(_this, this);
                    }).on('click', '.pagination a', function() {
                        _this.imageGet(this.href);
                        return !1;
                    });
                }
                if (_this.opts.imageGetJson) {
                    _this.imageGet();
                } else {
                    $("#redactor-modal-tab-2").remove();
                }

                if (_this.opts.imageUploadUrl !== null) {
                    if (!_this.isMobile() && !_this.isIPad()) {
                        if ($("#redactor_file").length) {
                            _this.draguploadInit("#redactor_file");
                        };
                    }

                    _this.uploadInit("redactor_file", {
                        auto: true,
                        url: _this.opts.imageUploadUrl,
                        success: $.proxy(_this.imageCallback, _this),
                        error: $.proxy(function(obj, json) {
                            _this.callback("imageUploadError", json);
                        }, _this)
                    });
                } else {
                    $(".redactor_tab").hide();
                ...
                if (!_this.opts.imageTabLink && (_this.opts.imageUploadUrl || _this.opts.imageGetJson)) {
                    $("#redactor-tab-control-3").hide();
                }
                $("#redactor_upload_btn").click($.proxy(_this.imageCallbackLink, _this));
                if (!_this.opts.imageUploadUrl && !_this.opts.imageGetJson) {
                    setTimeout(function() {
            ...
            _this.modalInit(_this.opts.curLang.image, _this.opts.modal_image, 746, callback);
        },
        imageGet: function(url) {
            var _this = this;
            $.getJSON(url || this.opts.imageGetJson).done($.proxy(function(data) {
                var html = '<ul class="redactor_image_box">',
                    list = '';
                if (data.code > 0) {
                    $.each(data.data.images, function(index, image) {
                        list += '<li data-url="' + image.url + '"><img src="' + image.url + '" alt="' + image.id + '" /></li>';
                    });
                    if (!list) html = '<p class="redactor_image_empty">暂无图片</p>';
                    else html += list + '</ul>';
                    $('#redactor_image_box').html(html + '<div class="redactor_image_pagination">' + data.data.pagination + '</div>');
                } else {
                    this.callback("imageGetError", {
                        code: 0,
                        msg: this.opts.curLang.image_get_net_error
                    });
                }
        ...
        imageClick: function(li) {
            var string = '<img id="image-marker" src="' + $(li).data('url') + '">';
            parent = this.getParent();
            if (this.opts.paragraphy && $(parent).closest("li").size() == 0) {
                string = "<p>" + string + "</p>";
            };
            this.imageInsert(string, true);
        },
        ...
        imageInsert: function(json, link) {
            ...
            if (json !== false) {
                ...
                if (link !== true) {
                    json = json.data;
                    html = "<img id='image-marker' src='" + json.url + "'>";
                    var parent = this.getParent();
                    if (this.opts.paragraphy && $(parent).closest("li").size() == 0) {
                        html = "<p>" + html + "</p>";
                    };
        ...
        hideProgressBar: function() {
            ...
            _this.$progressBar.stop(1, 1).fadeOut(1500, function() {
                _this.$progressBarSpan.width(0);
            });
        },
        modalTemplatesInit: function() {
            $.extend(this.opts, {
                ...
                modal_pre: String() + '<section id="redactor-modal-pre-insert"><form id="redactorInsertPreForm"><label>' + this.opts.curLang.pre_text_code + '</label><textarea id="redactor_insert_pre_area" style="width: 99%; height: 160px;"></textarea></form></section><footer><button class="redactor_modal_btn redactor_btn_modal_close">' + this.opts.curLang.cancel + '</button><button id="redactor_insert_pre_btn" class="redactor_modal_btn redactor_modal_action_btn">' + this.opts.curLang.insert + '</button></footer>',
        ...
        uploadInit: function(el, options) {
            ...
            var $el = $("#" + el);
            this.el = $el;
            $(this.el).change($.proxy(function(e) {
                this.uploadSubmit();
            }, this));
        },
        draguploadInit: function(file) {
            var $area, timeId = 0;
            if (window.FormData === undefined) {
                return false;
            };
            this.droparea = $('<div class="redactor_droparea"></div>');
            this.dropareabox = $("<div class='redactor_dropareabox'>" + this.opts.curLang.drop_file_here + "</div>");
            this.droparea.append(this.dropareabox);
            $(file).before(this.droparea);
            $area = this.droparea;
            $(document).on('dragover', function(e) {
                if ($(e.target).closest('#redactor_tab1').length) {
                    $area.css('z-index', 2);
                } else {
                    $area.css('z-index', 'auto');
                }
                return !1;
            })
            this.dropareabox.on("dragover", $.proxy(function() {
                ...
                return !1;
            }, this)).on("dragleave", $.proxy(function() {
        ...
        // 上传
        // 调用 jquery.fn.upload
        uploadSubmit: function(file) {
            var _this = this,
                $upload = file ? $('body') : _this.el;
            if ($.fn.upload) {
                _this.showProgressBar();
                $upload.upload({
                    action: _this.opts.imageUploadUrl,
                    headers: _this.opts.imageUploadHeaders,
                    data: _this.opts.imageUploadData,
                    name: _this.opts.imageUploadName,
                    onprogress: $.proxy(_this.uploadProgress, _this),
                    oncomplete: function() {
                        setTimeout(function() {
                    ...
                    onsuccess: function(json) {
                        ...
                        if (json.code > 0) {
                            $img = $("<img>");
                            $img.attr("src", json.data.url).attr("id", "drag-image-marker");
                            _this.execCommand("inserthtml", _this.outerHtml($img), false);
                            image = $(_this.$editor.find("img#drag-image-marker"));
                            if (image.length) {
                                image.removeAttr("id");
                            } else {
                                image = false;
                            ...
                            _this.observeImages();
                            if (image) {
                                _this.callback("imageUpload", image, json);
                            };
                        } else {
                            _this.callback("imageUploadError", json);
                        }
                    ...
                    onerror: function() {
                        _this.callback("imageUploadError", {
                            code: 0,
                            msg: _this.opts.curLang.image_upload_net_error
                        });
                    }
                });
                if (file) $upload.upload('file', file);
            } else {
                _this.callback("imageUploadError", {
                    code: 0,
                    msg: '未配置jquery-upload'
                });
            }
        },
        uploadProgress: function(e) {
            var percent = e.loaded ? parseInt(e.loaded / e.total * 100, 10) : 100;
            this.$progressBarSpan.width(percent + '%');
        },
        isMobile: function() {
