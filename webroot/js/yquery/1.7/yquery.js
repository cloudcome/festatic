...
(function(win, udf) {
    ...
    var doc = win.document,
        ...
        // 动画结束方法
        transitionendEventType = _html5(win, 'ontransitionend').slice(2),
        _requestAnimationFrame,
        _cancelRequestAnimationFrame,
    ...
    $.fn = Yquery.prototype = {
        ...
        css: function(key, val) {
            return this._getSet(_css3, key, val);
        },
        ...
        parent: function(isPositionParent) {
            return this._access(function() {
                if (_css3(this, 'position') === 'fixed' && isPositionParent) return $(doc);
        ...
        position: function() {
            return this._access(function() {
                ...
                if (!_isElement(element)) return {
                    top: 0,
                    left: 0
                };

                top = element.offsetTop - _parseFloat(_css3(element, 'marginTop'));
                left = element.offsetLeft - _parseFloat(_css3(element, 'marginLeft'));
        ...
        offset: function(offset) {
            if (offset) {
                return this.each(function() {
                    ...
                    cssLeft = _parseFloat($element.css('left'));
                    cssTop = _parseFloat($element.css('top'));

                    current = $element.offset();
                    if (offset.left === udf) offset.left = current.left;
                    if (offset.top === udf) offset.top = current.top;

                    if (offset.left !== current.left) _css3(element, 'left', cssLeft + offset.left - current.left);
                    if (offset.top !== current.top) _css3(element, 'top', cssTop + offset.top - current.top);
                });
            }
        ...
        bind: function(eventType, fn) {
            ...
            return this.each(function() {
                var element = this;
                _each(eventType, function(index, ev) {
         ...
         */
        tapick: function(fn) {
            return cantouch ? this.tap(fn) : this.click(fn);
        },
        ...
        on: function(eventType, selector, fn) {
            ...
            return this.each(function() {
                ...
                eventType.forEach(function(ev) {
                    if (~_inArray(ev, canNotBubbleEvents)) {
        ...
        _animate: function(to, duration, easing, callback, store, doCallback) {
            ...
            } else if (_type(duration) === 'function') {
                callback = duration;
                duration = 1;
                easing = dftEasing;
            } else if (_type(duration) === 'number') {
                switch (_type(easing)) {
                    case 'undefined':
                        easing = dftEasing;
                        callback = dftCallback;
                    ...
                    case 'string':
                        if (_type(callback) !== 'function') callback = dftCallback;
                        break;
                }
            }
            ...
            return the.each(function() {
                ...
                if (_type(to) === 'function') to = to.call(element);
        ...
        stop: function(isGoToEnd) {
            return this.each(function() {
                ...
                // stop true
                if (isGoToEnd) {
                    _css3(element, 'transition-duration', '');
                    _css3(element, 'transition-property', '');
                    _css3(element, 'transition-timing-function', '');
                    _store(element, 'animateTo', !1);
                    _store(element, 'animateCallback')();
                }
                // stop false
                else {
                    _css3(element, 'transition-duration', '.01ms');
                    _each(_store(element, 'animateTo'), function(key) {
                        _css3(element, key, _css3(element, key));
                    });
                }
            });
        }
        ...
        _listen(doc, 'touchstart', function(e) {
            if (e.touches && e.touches.length === 1) {
                var firstTouch = e.touches[0],
                    ...
                    toucheCallout = _css3(body, 'touch-callout'),
                    userSelect = _css3(body, 'user-select');

                _reset(e);
                _css3(body, 'touch-callout', 'none');
                _css3(body, 'user-select', 'none');
                x0 = firstTouch.pageX;
                y0 = firstTouch.pageY;
                t0 = Date.now();

                timeid = setTimeout(function() {
                    _css3(body, 'touch-callout', toucheCallout);
                    _css3(body, 'user-select', userSelect);
    ...
    ['In', 'Out'].forEach(function(val, key) {
        $.fn['fade' + val] = function(duration, easing, callback) {
            ...
            // in
                ...
                the.each(function() {
                    if (_css3(this, 'display') === 'none') {
                        _css3(this, 'display', _store(this, 'display') || 'block');
                    }
                });
                to.opacity = 1;
            }
    ...
    ['show', 'hide'].forEach(function(fnName, index) {
        $.fn[fnName] = function(duration, easing, callback) {
            ...
            if (index) {
                ...
                doCallback = function(cb) {
                    cb.call(this);
                    if (_parseFloat(the.css('width')) === 0 && _parseFloat(the.css('height')) === 0) the.css('display', 'none');
                };
            }
            // show
            else {
    ...
    ['', 'inner', 'outer', 'client'].forEach(function(prefix, index) {
        ['width', 'height'].forEach(function(suffix) {
            $.fn[index ? _toHumpString(prefix + '-' + suffix) : suffix] = function(value) {
                if (value === udf) {
                    return this._access(function() {
                        var element = this;
    ...
    $.extend({
        ...
        each: _each,
        inArray: _inArray,
        removeInArray: _removeInArray,
        toHumpString: _toHumpString,
        // toStriString: _toStriString,
        // 创建QueryString
    ...
    function _widthOrHeight(element, key, val) {
        ...
        if (element === win || element === doc) {
            ...
            if (_css3(element, 'display') === 'none') {
                _css3(element, 'display', 'block');
                firstNone = !0;
            }

            padding = isVertical ? _parseFloat(_css3(element, 'paddingTop')) + _parseFloat(_css3(element, 'paddingBottom')) : _parseFloat(_css3(element, 'paddingLeft')) + _parseFloat(_css3(element, 'paddingRight'));
            border = isVertical ? _parseFloat(_css3(element, 'borderTopWidth')) + _parseFloat(_css3(element, 'borderBottomWidth')) : _parseFloat(_css3(element, 'borderLeftWidth')) + _parseFloat(_css3(element, 'borderRightWidth'));
            margin = isVertical ? _parseFloat(_css3(element, 'marginTop')) + _parseFloat(_css3(element, 'marginBottom')) : _parseFloat(_css3(element, 'marginLeft')) + _parseFloat(_css3(element, 'marginRight'));
            boxSizing = _css3(element, 'boxSizing');
    ...
    function _parseElement(string) {
        var tempWrap = _createElement('div');
    ...
    function _class(element, classString, index) {
        ...
        if (!_isElement(element)) return ret;
    ...
    function _animateScroll(element, left, top, duration, callback) {
        var isTop = left === null,
            ...
            from = isTop ? _scroll(element, suffix) : _scroll(element, suffix),
            to = isTop ? top : left,
        ...
        (function _recursion() {
            ...
            if (pastTime >= duration || pastDistance >= totalDistance) {
                ...
                _requestAnimationFrame(function() {
                    ...
                    _scroll(element, suffix, from + flag * pastDistance);
                    _recursion();
                });
            }
        })();
    ...
    function _animate(element, to, duration, easing, callback, store) {
        ...
        _each(to, function(key, val) {
            if (regNumericCss.test(key) && _isNumeric(val, !0)) val += 'px';
            val += '';
            if (_css3(element, key) !== val) {
                newTo[key] = val;
                keys.push(key);
            }
        });
        ...
        // 空目标 或 隐藏元素 或 空目标
        if (_isEmptyObject(newTo) || _css3(element, 'display') === 'none' || !keys) return callback();
        ...
        (store || []).forEach(function(key) {
            _store(element, key, _css3(element, key));
        });

        // 监听，只执行最后一次回调
        if (!_store(element, 'animateHasCallback')) {
            _listen(element, transitionendEventType, function() {
        ...
        _css3(element, 'transition-duration', duration + 'ms');
        _css3(element, 'transition-property', keys.join(','));
        _css3(element, 'transition-timing-function', easing);

        setTimeout(function() {
            for (var i in to) {
                keys.push(_css3(element, i, to[i]));
            }
        }, 0);
    }
