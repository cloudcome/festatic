...
(function (win, undefined) {
	...
	_each({
		...
		yQuery.fn[method] = function (val) {
			...
			// set
				...
				for (; i < j; i++) {
					elem = this[i];
					sL = yQuery(elem).scrollLeft();
					sT = yQuery(elem).scrollTop();
					if (elem === win) {
						if (isTop) {
							sT = val;
						} else {
	...
	function _ondocready(fn) {
		// 如果文档已经准备完毕
		if (ondocready.isDocReady) {
			return fn();
		}
		...
		if (ondocready.timer) {
			// 则把回调添加到队列中
			ondocready.callback.push(fn);
		}
		// 不存在定时器，表明文档还未准备
		else {
			ondocready.callback = [fn];
			ondocready.timer = win.setInterval(_checkDocIsReady, 0);
		}
	}
	...
	function _checkDocIsReady() {
		...
		if (doc && doc.getElementsByTagName && doc.getElementById && doc.body) {
			var i = 0,
				l = ondocready.callback.length;

			console.info("yQuery is ready, past " + (new Date().getTime() - timestamp) + " ms.");

			// 标记为完毕
			ondocready.isDocReady = 1;

			// 清除检测定时器
			win.clearInterval(ondocready.timer);
			ondocready.timer = 0;

			// 处理回调函数
			for (; i < l; i++) {
				ondocready.callback[i]();
			}
