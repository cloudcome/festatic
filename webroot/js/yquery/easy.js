...
(function(window, undefined) {
    ...
    var easySelector = {
        ...
        adapter: function(selector, context, nextSelector) {
            ...
            type = nextSelector !== undefined ? 'RELATIVE' : ~selector.indexOf(':') ? 'PSEUDO' : ~selector.indexOf('#') ? 'ID' : ~selector.indexOf('[') ? 'ATTR' : ~selector.indexOf('.') ? 'CLASS' : 'TAG';
        ...
        indexFilter: function(name, i) {
            return name === 'even' ? i % 2 === 0 :
                name === 'odd' ? i % 2 === 1 : ~name.indexOf('n') ? (name === 'n' || i % parseInt(name) === 0) :
                parseInt(name) === i;
        }
    ...
    E.mix(E.prototype, {
        ...
        html: function(content) {
            ...
            if (E.isString(content) &&
                ...
                // 排除掉不能wrap的元素
                !rCssJsTag.test(content) && !wrapMap[(content.match(rTagName) || ['', ''])[1].toLowerCase()]) {
    ...
    ['padding', 'margin', 'borderWidth', 'borderRadius'].forEach(function(name) {
        ...
        animHooks[name] = {
            parse: function(val) {
                ...
                return {
                    val: {
                        top: parseFloat(val[0]),
                        right: parseFloat(val[2]),
                        bottom: parseFloat(val[4]),
                        left: parseFloat(val[6])
                    },
                    unit: {
                        top: val[1],
                        right: val[3],
