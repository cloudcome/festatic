...
/**
 * v1.0 2013年11月1日12:00:03
 * v1.1 2014年1月8日15:20:13
 */
...
(function ($, undefined) {
	...
	function _parse(strHash) {
		...
		// 删除所有#
		strHash = (strHash.match(/#([^#]*)$/) || ["", ""])[1];

		if (strHash === "") return objHash;

		// 删除开头多余的“!”和“?”
		strHash = strHash.replace(/^.*([!\?])/, "$1");

		// 取第一个字符
		hashType = strHash.charAt(0);

		// 如果不是 path 也不是 query 则返回
		if (hashType != "!" && hashType != "?") hashType = defaults.hashType;
